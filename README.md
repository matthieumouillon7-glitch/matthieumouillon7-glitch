## Hi there 👋

# MOUILLON  
*A passionate Digital Consultant and Data Scientist*

---

### 💼 À propos de moi
Je suis un passionné de tech, toujours curieux d’apprendre.  
Je collabore volontiers sur des projets Data Science, aussi bien comme chef de projet que datascientist.

Connectez-vous avec moi sur [LinkedIn](https://www.linkedin.com/in/matthieu-mouillon/).

---

### 🔗 Liens utiles
- [Portfolio](https://drive.google.com/drive/folders/1oE5kreDqci_MGAhVXBxNhUpdCczo2f4D)
- [Blog](https://www.human-station.com)
- Email : matthieumouillon7@gmail.com

---

### 🛠️ Compétences
`bash` &nbsp; `bootstrap` &nbsp; `docker` &nbsp; `fastapi` &nbsp; `figma` &nbsp;  
`gcp` &nbsp; `heroku` &nbsp; `huggingface` &nbsp; `illustrator` &nbsp; `jupyter` &nbsp;  
`keras` &nbsp; `langchain` &nbsp; `matplotlib` &nbsp; `mlflow` &nbsp; `mysql` &nbsp;  
`numpy` &nbsp; `pandas` &nbsp; `python` &nbsp; `pytorch` &nbsp; `scikit_learn` &nbsp;  
`seaborn` &nbsp; `streamlit` &nbsp; `tensorflow` &nbsp; `zapier`  

---

### 🌐 Réseaux sociaux
- GitHub : [matthieumouillon7-glitch](https://github.com/matthieumouillon7-glitch)
- LinkedIn : [matthieu-mouillon](https://www.linkedin.com/in/matthieu-mouillon/)
- Kaggle : [matthieumouiilon](https://www.kaggle.com/matthieumouiilon)

---

<!--
Ton json profil ici pour exporter des données, ne pas afficher sur le README
-->
